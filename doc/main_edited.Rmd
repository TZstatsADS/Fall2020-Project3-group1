...
if(run.cv.RF){
  ...
  for(i in 1:length(ntree)){
    ...
    cat("mtry = ", mtry[j], "\n")
    res_cv[(i-1)*length(mtry)+j,] <- cv.function(features = feature_train, labels = label_train, K, ntree = ntree[i],mtry=mtry[j])
  }}
...
# Mean Error
# ntrees = 100
ggplot(res_cv_rforest_cv_results[res_cv_rforest_cv_results$ntrees == 100, ],
       aes(x = as.factor(maxd), y = mean_error, 
           ymin = mean_error - sd_error, ymax = mean_error + sd_error)) + 
  geom_crossbar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))

# ntrees = 300
ggplot(res_cv_rforest_cv_results[res_cv_rforest_cv_results$ntrees == 300, ],
       aes(x = as.factor(maxd), y = mean_error, 
           ymin = mean_error - sd_error, ymax = mean_error + sd_error)) + 
  geom_crossbar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))

# ntrees = 500
ggplot(res_cv_rforest_cv_results[res_cv_rforest_cv_results$ntrees == 500, ],
       aes(x = as.factor(maxd), y = mean_error, 
           ymin = mean_error - sd_error, ymax = mean_error + sd_error)) + 
  geom_crossbar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
```{r, echo = FALSE}
cat("The accuracy of the random forest with weights model (", "ntrees = ", par_best_res_cv_xgboost_cv_results_ntrees, 
     ", max_depth = ", par_best_res_cv_xgboost_cv_results_md, 

    ") is ", accu*100, "%.\n", sep = "")
cat("The AUC of the random forest with weights  model (", "ntrees = ", par_best_res_cv_xgboost_cv_results_ntrees, 
    ", max_depth = ", par_best_res_cv_xgboost_cv_results_md, 
    ") is ", auc, ".\n", sep = "")
```
...
```{r running_time_rforestw, echo = FALSE}
cat("Time for training random forest with weights model = ", tm_train_rforestw[1], " seconds \n", sep = "") 
cat("Time for testing random forest with weights model = ", tm_test_rforestw[1], " seconds \n" , sep = "")
```
