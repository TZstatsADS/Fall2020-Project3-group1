...
if (run.train.gbm) {
  ...
  if (sample.reweight){
    tm_train_gbm <- system.time(fit_train <- train(feature_train, label_train, w = weight_train, 
                                               num_trees = par_best_gbm_n.trees, shrink = par_best_gbm_shrinkage))
  } else {
    tm_train_gbm <- system.time(fit_train <- train(feature_train, label_train, w = NULL, 
                                               num_trees = par_best_gbm_n.trees, shrink = par_best_gbm_shrinkage))
  }
...
##### Gradient Boosted Trees (gbm) (Baseline Model)
...
if(run.test.gbm){
  load(file="../output/fit_train_gbm.RData")
  tm_test_gbm <- system.time({prob_pred <- test(fit_train, feature_test, pred.type = 'response'); 
...
### add run times for xgboost model here
