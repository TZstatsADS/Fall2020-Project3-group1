...
```{r model_setup}
nprinciple = c(360,500,650,800,950,1100,1250,1400,1550,1700,1850,2000,2150,2300,2400)
# nprinciple = c(650)
model_labels = paste("PCA principle components", nprinciple)
```
...
```{r runcv}
if(run.cv){
  res_cv <- matrix(0, nrow = length(nprinciple), ncol = 4)
  for(i in 1:length(nprinciple)){
    cat("Number of principle component = ", nprinciple[i], "\n")
    res_cv[i,] <- cv.function(features = feature_train, labels = label_train, K,pca1,
                              np=nprinciple[i], reweight = sample.reweight)
  save(res_cv, file="../output/res_cv_svm.RData")
  }
}else{
...
# colnames(res_cv) <- c("mean_error", "sd_error", "mean_AUC", "sd_AUC","mean_accu","sd_accu")
colnames(res_cv) <- c("mean_accu","sd_accu")
res_cv$k = as.factor(nprinciple)

if(run.cv){
  # p1 <- res_cv %>% 
  #   ggplot(aes(x = as.factor(nprinciple), y = mean_error,
  #              ymin = mean_error - sd_error, ymax = mean_error + sd_error)) + 
  #   geom_crossbar() +
  #   theme(axis.text.x = element_text(angle = 90, hjust = 1))
  # 
  # p2 <- res_cv %>% 
  #   ggplot(aes(x = as.factor(nprinciple), y = mean_AUC,
  #              ymin = mean_AUC - sd_AUC, ymax = mean_AUC + sd_AUC)) + 
  #   geom_crossbar() +
  #   theme(axis.text.x = element_text(angle = 90, hjust = 1))
  p3 <- res_cv %>% 
    ggplot(aes(x = as.factor(nprinciple), y = mean_accu,
...
```{r best_model}
par_best <- nprinciple[which.max(res_cv$mean_accu)] 
```
...
# }
tm_train <- system.time(fit_train <- train(feature_train, label_train,pca1,par_best))
...
if(run.test){
  ...
  tm_test <- system.time({ prob_pred <- test(fit_train, feature_test, pca1,par_best)})
}
```
